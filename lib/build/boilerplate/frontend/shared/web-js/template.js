...
export const generateWebJSTemplate = ({ configType, isFullStack, userArguments }) => {
    ...
    if (userArguments?.firstfactors || userArguments?.secondfactors) {
        ...
        if (factors.some((f) => f.startsWith("otp-") || f.startsWith("link-"))) recipesSet.add("passwordless"); // Added type
        ...
        if (userArguments.secondfactors?.some((f) => f.includes("email"))) recipesSet.add("emailVerification"); // Added type
        ...
        configToRecipes[configType]?.forEach((recipe) => recipesSet.add(recipe)); // Added type, Add null check for safety
    }
    ...
    const needsEmailVerification =
        hasMFA &&
        userArguments?.secondfactors?.some((factor) => factor.includes("email")) && // Added type
