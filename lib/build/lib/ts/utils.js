...
async function setupFrontendBackendApp(answers, folderName, locations, userArguments, spinner) {
    ...
    for (const config of selectedBackend.location.config) {
        // Copy the shared recipe config based on the backend language
        const sharedConfigPath = `boilerplate/backend/shared/${backendLang}/recipes/${answers.recipe}.${
            backendLang === "go" ? "go" : backendLang === "python" ? "py" : "ts"
        }`;
        fs.copyFileSync(sharedConfigPath, `${folderName}/backend/${normaliseLocationPath(config.finalConfig)}`);
...
async function setupFullstack(answers, folderName, userArguments, spinner) {
    ...
    for (const config of selectedFullStack.location.config.backend) {
        ...
        fs.copyFileSync(sharedConfigPath, `${folderName}/${normaliseLocationPath(config.finalConfig)}`);
        // Remove the original configs folder if it exists
