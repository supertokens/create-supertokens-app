...
async function performAdditionalSetupForFrontendIfNeeded(selectedFrontend, selectedBackend, folderName, userArguments) {
    /**
    ...
    if (doesUseAuthReact) {
        ...
        if (selectedBackend.value === "go-http" ||
...
async function setupFrontendBackendApp(answers, folderName, locations, userArguments, spinner) {
    ...
    for (const config of selectedFrontend.location.config) {
        ...
        fs.copyFileSync(`${folderName}/frontend/${normaliseLocationPath(config.configFiles)}/${frontendRecipeConfig[0]}`, `${folderName}/frontend/${normaliseLocationPath(config.finalConfig)}`);
    ...
    for (const config of selectedBackend.location.config) {
        ...
        fs.copyFileSync(`${folderName}/backend/${normaliseLocationPath(config.configFiles)}/${backendRecipeConfig[0]}`, `${folderName}/backend/${normaliseLocationPath(config.finalConfig)}`);
        // Remove the configs folder
...
async function setupFullstack(answers, folderName, userArguments, spinner) {
    ...
    for (const config of selectedFullStack.location.config.frontend) {
        ...
        fs.copyFileSync(`${folderName}/${normaliseLocationPath(config.configFiles)}/${frontendRecipeConfig[0]}`, `${folderName}/${normaliseLocationPath(config.finalConfig)}`);
        // Remove the configs folder
    ...
    for (const config of selectedFullStack.location.config.backend) {
        ...
        fs.copyFileSync(`${folderName}/${normaliseLocationPath(config.configFiles)}/${backendRecipeConfig[0]}`, `${folderName}/${normaliseLocationPath(config.finalConfig)}`);
        // Remove the configs folder
        fs.rmSync(`${folderName}/${normaliseLocationPath(config.configFiles)}`, {
            recursive: true,
            force: true,
