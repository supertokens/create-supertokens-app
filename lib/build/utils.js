...
async function setupFrontendBackendApp(answers, folderName, locations, userArguments) {
    var _a, _b;
    ...
    const frontendSetup = new Promise((res) => {
        ...
        (_a = setup.stderr) === null || _a === void 0 ? void 0 : _a.on("data", data => {
            // Record any messages printed as errors
            stderr.push(data.toString());
        });
        (_b = setup.stdout) === null || _b === void 0 ? void 0 : _b.on("data", data => {
            /**
    ...
    const rootSetup = new Promise((res) => {
        ...
        (_a = rootInstall.stderr) === null || _a === void 0 ? void 0 : _a.on("data", data => {
            // Record any messages printed as errors
            stderr.push(data.toString());
        });
        (_b = rootInstall.stdout) === null || _b === void 0 ? void 0 : _b.on("data", data => {
            /**
             * Record any messages printed as errors, we do this for stdout
             * as well because some scripts use the output stream for errors
             * too (npm for example) while others use stderr only
...
async function setupFullstack(answers, folderName, userArguments) {
    ...
    fs.copyFileSync(`${folderName}/${normaliseLocationPath(selectedFullStack.location.config.frontend.configFiles)}/${frontendRecipeConfig[0]}`, `${folderName}/${normaliseLocationPath(selectedFullStack.location.config.frontend.finalConfig)}`);
    // Remove the configs folder
    ...
    fs.copyFileSync(`${folderName}/${normaliseLocationPath(selectedFullStack.location.config.backend.configFiles)}/${backendRecipeConfig[0]}`, `${folderName}/${normaliseLocationPath(selectedFullStack.location.config.backend.finalConfig)}`);
    // Remove the configs folder
...
export async function setupProject(locations, folderName, answers, userArguments) {
    const isFullStack = locations.frontend === locations.backend;
    if (!isFullStack) {
