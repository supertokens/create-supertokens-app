...
import { appInfo } from "~/config/appInfo"; // Adjust path as necessary
...
export function handleAuthAPIRequest(NuxtResponse: typeof Response) {
    ...
    return async function handleCall(req: Request) {
        ...
        if (!handled) {
            return new NuxtResponse("Not found", { status: 404 });
        }
        ...
        return new NuxtResponse(baseResponse.body, {
...
function createPreParsedRequest(request: Request) {
    ...
    try {
        ...
        if (Session.Error.isErrorFromSuperTokens(err)) {
            return {
                ...
                session: undefined,
                baseResponse,
                nuxtResponse: new Response("Authentication required", {
...
export async function getSessionForSSR(nuxtRequest: Request): Promise<{
    accessTokenPayload: JwtPayload | undefined;
    hasToken: boolean;
    error: Error | undefined;
}> {
    const accessToken = getAccessToken(nuxtRequest);
...
export async function withSession(
    ...
    try {
        const baseRequest = createPreParsedRequest(nuxtRequest);
        const { session, nuxtResponse, baseResponse } = await getSessionDetails(baseRequest, options, userContext);

        if (nuxtResponse !== undefined) {
            return nuxtResponse;
        }
