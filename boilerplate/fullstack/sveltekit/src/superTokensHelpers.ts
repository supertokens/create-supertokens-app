...
export function handleAuthAPIRequest(svelteResponse: typeof Response) {
    ...
    return async function handleCall(req: Request) {
        ...
        if (!handled) {
            return new svelteResponse("Not found", { status: 404 });
        }
        ...
        return new svelteResponse(baseResponse.body, {
    ...
    try {
        ...
        if (Session.Error.isErrorFromSuperTokens(err)) {
            return {
                ...
                session: undefined,
                baseResponse,
                svelteResponse: new Response("Authentication required", {
 ...
 */
export async function getSessionForSSR(svelteRequest: Request): Promise<{
    accessTokenPayload: JwtPayload | undefined;
    hasToken: boolean;
    error: Error | undefined;
}> {
    const accessToken = getAccessToken(svelteRequest);
...
export async function withSession(
    svelteRequest: Request,
    handler: (error: Error | undefined, session: SessionContainer | undefined) => Promise<Response>,
    options?: VerifySessionOptions,
    userContext?: Record<string, any>
): Promise<Response> {
    try {
        const baseRequest = createPreParsedRequest(svelteRequest);
        const { session, svelteResponse, baseResponse } = await getSessionDetails(baseRequest, options, userContext);

        if (svelteResponse !== undefined) {
